...
class MainGame:
    def __init__(self, display, gamestatemanager):
        ...
        # Load background and design images
        ...
        self.questions_category_0 = self.math_questions.questions_by_category[0]
        random.shuffle(self.questions_category_0)

        self.current_question_index = -1

        # Input and Music setup
    ...
    def run(self):
        ...
        for event in pygame.event.get():
            ...
            if event.type == pygame.KEYDOWN and self.start_time is not None and time.time() - self.start_time < self.countdown_duration:
                if self.active:
                    if event.key == pygame.K_RETURN:
                        if self.text.strip():
                            if 0 <= self.current_question_index < len(self.questions_category_0):
                                _, correct_answer = self.questions_category_0[self.current_question_index]
            ...
            if event.type == pygame.KEYDOWN:
                if event.key == pygame.K_SPACE and self.start_time is None:
                    ...
                    self.designright_rect.center = (3 * self.display.get_width() // 2.97, self.display.get_height() // 2.5)
        ...
        if self.start_message:
            ...
            if self.start_time is not None:
                ...
                if self.current_question_index < len(self.questions_category_0):
    ...
    def display_question(self):
        if self.start_time is not None and time.time() - self.start_time < self.countdown_duration:
            if self.current_question_index >= 0 and self.current_question_index < len(self.questions_category_0):
                font = pygame.font.Font('Fonts/SuperSalad-qZgvV.ttf', 25)
                question, _ = self.questions_category_0[self.current_question_index]
