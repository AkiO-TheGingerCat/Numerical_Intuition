...
class HardGame:
    def __init__(self, display, gamestatemanager):
        ...
        self.questions_category_2 = self.math_questions.questions_by_category[2]
        random.shuffle(self.questions_category_2)
    ...
    def run(self):
            ...
            if event.type == pygame.KEYDOWN and self.start_time is not None and time.time() - self.start_time < self.countdown_duration:
                if self.active:
                    if event.key == pygame.K_RETURN:
                        if self.text.strip():
                            if 0 <= self.current_question_index < len(self.questions_category_2):
                                _, correct_answer = self.questions_category_2[self.current_question_index]
        ...
        if self.start_message:
            ...
            if self.start_time is not None:
                ...
                if self.current_question_index < len(self.questions_category_2):
    ...
    def display_question(self):
        if self.start_time is not None and time.time() - self.start_time < self.countdown_duration:
            if self.current_question_index >= 0 and self.current_question_index < len(self.questions_category_2):
                font = pygame.font.Font('Fonts/SuperSalad-qZgvV.ttf', 25)
                question, _ = self.questions_category_2[self.current_question_index]
